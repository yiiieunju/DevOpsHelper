package com.ktds.devopshelper.gatewayserver.filter;
...
import com.ktds.devopshelper.gatewayserver.service.AccountService;
...
public class TokenCheckFilter implements WebFilter{
	...
	public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
		...
		Object tmp2 = request.getHeaders().get("userId");
		String token = "";
		String userId = "";
		...
		if (tmp2 != null) {
			userId = tmp2.toString().replace("[", "").replace("]", "");
		}
		
		log.info("token = {}" , token);
		
		success = accountService.existsByUserIdAndToken(userId, token);
